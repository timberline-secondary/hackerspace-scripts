...
while True:
    image_link = input("Paste your image link (must start with https), or specify exact directory with extension (.png,.jpg,etc..) \n")
    if image_link == 'q':
        break
    ext = image_link[-4:]
    if image_link[0:2] == "~/":
    ...
    student_number = input("Enter Studend Number\n")

    image_name = input("Name your image (MUST NAME IF YOU USED URL) or press Enter to keep orriginal\n")
